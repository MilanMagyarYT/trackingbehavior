 ...
 */
export function computeDailyMetrics(docs: DocumentData[]) {
    // 1) Group by timeBucket
  const buckets = { morning: [], afternoon: [], evening: [], night: [] } as Record<string, DocumentData[]>;
  docs.forEach(d => buckets[d.timeBucket].push(d));

  // 2) For each bucket calculate your 14 metrics
  const periodMetrics = Object.fromEntries(
    Object.entries(buckets).map(([period, arr]) => [
      period,
      {
        simple: {
          ...
          topTrigger: (() => {
            ...
            arr.forEach(x => x.triggers.forEach((t: string) => freq[t] = (freq[t]||0)+1));
